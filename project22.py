...
queries={
    "1. top spending customers":
     """select Customer_ID,SUM(Order_Value) AS total_spent from analysis group by Customer_ID order by total_spent desc limit 10""",
    ...
    "4.Monthly revenue trends":
     """select extract( month from Order_Date) as month,SUM(Order_Value) AS total_revenue,COUNT(*) AS total_orders from analysis 
...
# sidebar
page=st.sidebar.radio('Dashboard',['About','Queries','Home'])
if page=='About':
 st.title('Online Food Delivery Analysis Dashboard')
 st.image("https://t4.ftcdn.net/jpg/03/94/73/73/240_F_394737308_A5IJf7vijvkGWCsiCcNI1kAGWoa5g54h.jpg",width=600)
 st.write("Food delivery analytics involves analyzing data from apps to  optimize operations, enhance customer experience, and increase profitability. Key applications include predicting delivery times based on traffic/weather, optimizing routes, managing inventory,analyzing customer preferences for targeted marketing, and monitoring restaurant performance.")


 
if page== 'Queries':
 task = st.sidebar.selectbox("choose query number", list(queries.keys()))
 st.sidebar.button("run query")
 query=queries[task]
...
if page=='Home':
    st.markdown(
    "<p style='color:skyblue;font-size:40px;'>Welcome to the home page</p>",
    unsafe_allow_html=True
) 
    st.balloons()
    st.success('successfully completed our second project')